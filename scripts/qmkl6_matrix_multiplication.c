...
int write_data(int row) {
    char *cmd = (char*)"sudo rdserialtool --device=um25c --bluetooth-address=00:16:A6:00:13:2B --quiet --json";
...
int main(void) {
  ...
  for (int i = 64; i <= 2048; i = i * 2) {
    ...
    const int m = rows, n = rows, k = columns;
    const int ldc = n;
    const float alpha = 2.f, beta = 3.f;
    const int lda = k;
    const int ldb = n;
    ret = test_sgemm_single(CblasRowMajor, CblasNoTrans, CblasNoTrans, m, n, k, alpha, lda, ldb, beta, ldc, gen);
    write_data(rows);
  }
